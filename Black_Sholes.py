...
def simulate_Black_Scholes(S0, mu, sigma, T, dt) -> pd.DataFrame:
    ...
    time, delta_t = np.linspace(0, T, num = N+1, retstep = True) # Calculate time increments for output index
    
    S = np.exp((mu - sigma ** 2 / 2) * dt + sigma * np.random.normal(0, np.sqrt(dt), size= N)) # Generate sample path increments for each time period
    S = np.hstack([1, S]) # Stack all increments in a single row, starting with the trivial increment at time 0 
    S = S0* S.cumprod(axis=0) # Multiply relative increments and multiply by stock price
